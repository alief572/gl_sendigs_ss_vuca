	...
	<form action="#" method="POST" id="form_data">
		<div class="box box-primary">
			<!-- <div class="box-header">
				<h3 class="box-title">Report Multi BUM</h3>
			</div> -->

			<div class="alert alert-info">
				<form action="<?php echo site_url() ?>index.php/Multi_bum" method="POST" id="form-proses-bro">
					<!-- <form action="<?php echo site_url() . '/multi_bum'; ?>" method="POST" id="form-proses-bro"> -->
					<div class="row">
						<div class="col-sm-10">
							<div class="col-sm-3">
				...
				<table id="example11" class="table table-striped table-bordered table-hover" width="100%">
					<thead>
						<tr bgcolor='#9acfea'>
							...
							<th class="no-sort text-center">Terima Dari</th>
							<!-- <th class="no-sort text-center">Referensi</th> -->
					...
					<tbody>
						...
						if ($rows_data) {
							...
							foreach ($rows_data as $keyR => $valR) {
								...
								$data_uang_terima_dari = $this->db->query("SELECT * FROM jurnal WHERE tipe='BUM' and nomor='$valR->nomor' and no_perkiraan like '%1101-02-01%'")->result();

								if ($data_uang_terima_dari > 0) {
									foreach ($data_uang_terima_dari as $row_jurnal1) {
										$uang_terima_dari = $row_jurnal1->no_perkiraan;
										...
										$data_buk_coa	= $this->db->query("SELECT * FROM coa WHERE no_perkiraan = '$uang_terima_dari' and bln='$bln' and thn='$thn' and kdcab='$kode_cabang'")->result();
										if ($data_buk_coa > 0) {
								...
								echo "<td class='text-left'>" . $valR->terima_dari . "</td>";
								// echo "<td class='text-left'>" . $valR->no_reff . "</td>";
			...
			<div class="modal-content" style="border-radius: 8px; overflow: hidden; box-shadow: 0 5px 25px rgba(0,0,0,0.2);">
				<div class="modal-header" style="background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); color: #fff; padding: 15px 20px;">
					...
					<h4 class="modal-title" id="Mymodal-title" style="font-weight: 600; font-size: 16px;">
						<i class="fa fa-file-text-o" style="margin-right: 8px;"></i> Detail Data BUM
					</h4>
				</div>
	...
	<script type="text/javascript">
		...
		$(document).on('click', '#PrintData', function(e) {
			...
			if (dataCheck == false) {
				...
				window.open("<?php echo site_url('multi_bum/print_bum/?no_bum='); ?>" + dataCheck, "_blank");
			}
		});

		function view_detail(kode) {
			$.ajax({
				url: base_url + 'index.php/' + active_controller + '/detail_bum/' + kode,
				cache: false,
				type: "GET",
